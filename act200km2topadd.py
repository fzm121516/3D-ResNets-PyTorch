...
def process_video(model, spatial_transform, video_path, images_dir, args):
    ...
    total_clips = 0
    aggregated_outputs = torch.zeros(1, 1039)
    ...
    aggregated_outputs /= total_clips
    confidence, predicted_class = torch.max(aggregated_outputs, 1)
    confidence = confidence.item()
    predicted_class = predicted_class.item()

    # Determine if this video is walking based on the highest confidence clip
    is_walking_video = predicted_class in {855, 354}
    has_walking = is_walking_video

    return total_clips, has_walking, is_walking_video, True, confidence
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=args.num_workers) as executor:
        ...
        for future in as_completed(futures):
            num_clips, has_walking, is_walking_video, processed, confidence = future.result()
            if processed:
                ...
                if is_walking_video:
                    walking_videos += 1
                print(f"Video processed. Confidence: {confidence:.4f}")
